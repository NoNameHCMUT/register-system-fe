...
import { Search, MapPin, Calendar, SlidersHorizontal, ChevronDown, ChevronLeft, ChevronRight, Bell, User } from "lucide-react";
...
const campaigns = [
  {
    ...
    slotValue: 75,
    image: "/assets/campaigns/education_campaign_1775894477648.png",
  },
  {
    id: 2,
    ...
    <div className="flex min-h-svh flex-col bg-[#f7f9fc]">
      ...
      <header className="border-b border-[#e6eaf0] bg-white sticky top-0 z-50 h-[72px]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between relative h-full">
          ...
          <nav className="hidden md:flex space-x-8 absolute left-1/2 top-0 bottom-0 -translate-x-1/2">
            <a href="#" className="flex items-center h-full text-[#2890d4] border-b-2 border-[#2890d4] font-semibold px-2">Campaigns</a>
            <a href="#" className="flex items-center h-full text-gray-500 hover:text-gray-900 font-medium px-2 border-b-2 border-transparent hover:border-gray-200">Activities</a>
            <a href="#" className="flex items-center h-full text-gray-500 hover:text-gray-900 font-medium px-2 border-b-2 border-transparent hover:border-gray-200">Volunteers</a>
            <a href="#" className="flex items-center h-full text-gray-500 hover:text-gray-900 font-medium px-2 border-b-2 border-transparent hover:border-gray-200">Map</a>
          </nav>
      ...
      <section className="bg-gradient-to-br from-[#2890d4] to-[#00c7d4] text-white py-16 px-4 relative overflow-hidden">
        <div className="max-w-7xl mx-auto relative z-10">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-4 max-w-2xl">
            Ignite Change,<br />One Project at a Time.
          </h1>
          <p className="text-lg md:text-xl font-light mb-8 max-w-2xl text-white/90">
            Join thousands of students across the nation in the "Mua He Xanh" movement.
            Find campaigns that match your skills and passion for community service.
          </p>
      ...
      <section className="max-w-7xl mx-auto px-4 -mt-6 sm:-mt-8 relative z-10 w-full mb-12">
        <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="col-span-1">
            <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wider">District</label>
            <div className="relative">
              <select className="w-full appearance-none bg-gray-50 border border-transparent hover:border-gray-200 text-gray-800 py-3 pl-4 pr-10 rounded-md focus:outline-none focus:ring-2 focus:ring-[#2890d4]">
          ...
          <div className="col-span-1 md:col-span-1 border-gray-200">
            <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wider">Campaign Period</label>
            <div className="relative">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 w-full mb-16">
        <div className="flex justify-between items-end mb-6">
          ...
          <p className="text-gray-500 font-medium text-sm">Showing 128 opportunities</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {campaigns.map((campaign) => (
            <div key={campaign.id} className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row overflow-hidden hover:shadow-md transition-shadow">
              {/* Image Side */}
              <div className="w-full sm:w-2/5 min-h-[200px] relative">
                ...
                <div className="absolute top-4 left-4">
                  <span className={`text-white text-xs font-bold px-3 py-1 rounded-full ${campaign.categoryColor.startsWith('bg-[') ? campaign.categoryColor : campaign.categoryColor}`}>
                    {campaign.category}
                  </span>
                </div>
              </div>
              ...
              <div className="w-full sm:w-3/5 p-6 flex flex-col justify-between">
                <div>
                  <h4 className="text-[#2890d4] text-xs font-bold uppercase tracking-wider mb-2">{campaign.university}</h4>
                  <h3 className="text-xl font-bold text-gray-900 mb-4 leading-tight">{campaign.title}</h3>
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {campaign.tags.map(tag => (
                      <span key={tag} className="bg-gray-100 text-gray-600 text-xs font-semibold px-2.5 py-1 rounded">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  <div className="flex justify-between items-center text-xs font-semibold mb-2">
                    <span className="text-gray-500 uppercase tracking-wider">Volunteer Slots</span>
                    <span className="text-[#2890d4]">{campaign.slotsConfig}</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-1.5 mb-4">
                    <div className="bg-[#2890d4] h-1.5 rounded-full" style={{ width: `${campaign.slotValue}%` }}></div>
                  </div>
        ...
        <div className="flex justify-center mt-12 mb-8">
          <nav className="flex items-center gap-1">
            <button className="p-2 text-gray-400 hover:text-gray-600"><ChevronLeft size={20} /></button>
            <button className="w-10 h-10 flex border border-[#2890d4] items-center justify-center rounded bg-[#2890d4] text-white font-medium">1</button>
            <button className="w-10 h-10 flex items-center justify-center rounded text-gray-600 hover:bg-gray-100 font-medium">2</button>
            <button className="w-10 h-10 flex items-center justify-center rounded text-gray-600 hover:bg-gray-100 font-medium">3</button>
            <span className="w-10 h-10 flex items-center justify-center text-gray-400">...</span>
            <button className="w-10 h-10 flex items-center justify-center rounded text-gray-600 hover:bg-gray-100 font-medium">12</button>
